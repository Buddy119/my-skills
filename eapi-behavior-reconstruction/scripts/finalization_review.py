...
FINALIZATION_REVIEW_VALIDATION_VERSION = "1"
...
def load_review_schema(path: Path | None = None) -> dict[str, Any]:
    ...
    if payload.get("finalization_review_schema_version") != "1":
...
def record_review(
    *,
    ...
    if path.is_file():
        ...
        ledger = {
            "finalization_review_schema_version": "1",
    ...
    if (
        ledger.get("finalization_review_schema_version") != "1"
...
def evaluate_reviews(
    *,
    ...
    if ledger.get("finalization_review_schema_version") != "1" or ledger.get(
        "validation_version"
