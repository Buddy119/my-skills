...
FINALIZATION_REVIEW_VALIDATION_VERSION = "3"
...
def load_review_schema(path: Path | None = None) -> dict[str, Any]:
    ...
    if payload.get("finalization_review_schema_version") != "3":
...
def record_review(
    *,
    ...
    if path.is_file():
        ...
        ledger = {
            "finalization_review_schema_version": "3",
    ...
    if (
        ledger.get("finalization_review_schema_version") != "3"
...
def evaluate_reviews(
    *,
    ...
    if ledger.get("finalization_review_schema_version") != "3" or ledger.get(
        "validation_version"
