...
Before modeling Behavior Flow, Implementation Sequence, exception propagation, Java implementation slices, or Config-to-Endpoint impacts, read [references/developer-implementation-trace-policy.md](references/developer-implementation-trace-policy.md) completely.
...
│   ├── java-implementation-map.md           # Java only, when executable bindings exist
...
The generated Migration Plan is the complete scope contract. It uses only `preserve`, `mechanical-migrate`, `archive-and-rebuild`, and `block`, and records the earliest safe recovery stage. The top-level recovery stage must never be later than any effective Step or invalidated Artifact's `rebuilding_stage`; any Dossier rebuild therefore returns to `tracing`, or `inventory` when Evidence is missing. Treat a conflicting Plan as invalid and rerun Resume Audit rather than editing it. Every mechanical step declares its Transform ID, exact source/target identity, paths, ID/link rules, expected counts, Manifest policy, and referential checks. There is no `review-and-adopt` action. Do not add an undeclared migration because a document "looks old". Old BA directories, stale Contracts, and other incompatible reader files are archived only by the Migration transaction, never as a side effect of `business-model` or another publication stage.
...
5. Follow the executable path from trigger through input handling, validation, decisions, data access, external boundaries, outputs, and material failures. Build Behavior Flow and Implementation Sequence as independent models: the first owns result-changing decisions and outcomes; the second owns participants, ordered calls and returns, boundaries, transaction position, asynchronous handoff, and exception propagation. Record raw `LIFE-OBS-*` facts, then keep object conditions, processing actions, and data movement in separate dossier sections. Do not call an action or location a State.
...
9. Update the relevant sections of `.work/repository-register.md` while the evidence is in context. Record each lifecycle fact first as an Observation; do not create a Transition without supported From/To object conditions and a real change point. Record each executable external boundary as a Dependency Observation and each material failure path as a Failure Observation, including origin, handler, propagation, visible result, state, and recovery. Record behavior-changing configuration reads as `CFG-OBS-*`. For Java, record exact candidate types, edges, and implementation bindings without selecting an unresolved runtime implementation. Do not create reader-facing Dependency Contracts or Failure Patterns while tracing one Behavior.
...
16. Reconcile `CFG-OBS-*` into stable `CFG-*` identities and `CFG-*-I*` impacts. Link an Endpoint only through a confirmed Config read or wiring → executable Behavior effect → Behavior/Endpoint chain. Keep exposure configuration in Endpoint reconciliation.
17. For Java, reconcile executable production types into `JTYPE-*`, allowed dependency edges into `JEDGE-*`, and Behavior/Endpoint implementation slices into `JIMPL-*`. Confirm DI or runtime selection separately from static symbol edges and retain dynamic/generated boundaries.
18. Build repository-level Behavior Flow, Implementation Sequence, and Exception Handling models from the reviewed dossiers. Do not reuse one flow's node metadata for the other.
19. Build the `Repository connection model` from reconciled endpoint, dependency, lifecycle, configuration, and failure models. Group only when participant/resource, direction, boundary type, interaction role, and configuration-selection semantics are equivalent. Include only executable crossings or explicit trigger bindings; a class, host, resource, or configuration name alone is not a connection.
20. Build the `Shared behavior model`. Include a rule or behavior-shaping component only when the same proven source is reused by at least two Behaviors or independent entry paths and materially changes validation, decisions, authorization, transformation, state, boundaries, output, error handling, or recovery. Preserve behavior-specific differences and overrides. Exclude logging, ordinary monitoring, generated code, framework glue, simple wrappers, and single-Behavior helpers.
21. Explain blocked coverage, conflicts, and unknowns instead of filling gaps with intent.
22. Complete the synthesis checkpoints in their executor-provided order: Endpoint, Outbound HTTP, Dependency, Failure, Lifecycle/Config, Connection/Shared Model, and synthesis review. Commit with `--semantic-result complete` only after every Lifecycle, Dependency, Failure, Config, and applicable Java Observation is reconciled or explicitly unresolved, every Transition and both Behavior models received semantic review, and the repository models are complete and reviewed. This commit advances only the Working Generation; it does not replace the formal Pack.
...
1. Scaffold one `tech-behavior` per completed dossier with `--identity behavior_id=<behavior-id>`, then write the developer-facing Behavior. Publish the independently modeled Behavior Flow (`flowchart`) and Implementation Sequence (`sequenceDiagram`) as peer Reader views. The sequence shows only behavior-relevant participants, calls, returns, boundaries, transaction position, and exception propagation—not a complete call graph. Add the Behavior-specific Exception Handling trace and keep repository-wide grouping in Failure Taxonomy.
...
4. Scaffold only applicable repository references and populate them from the corresponding reconciled register records and repository-synthesis models:
   ...
   - [assets/java-implementation-map-template.md](assets/java-implementation-map-template.md) for applicable Java repositories
...
9. Build Runtime Config from reconciled `CFG-*` identities and `CFG-*-I*` impacts. Include an Endpoint reverse index that states the condition, execution difference, caller/state/failure effect, and links to the owning Behavior, Contract, and Failure detail. Do not treat deployment intent as application behavior.
10. For Java repositories with reconciled bindings, build one `java-implementation-map.md`: define shared `JTYPE-*` once, organize `JEDGE-*` beneath `JIMPL-*` slices, link Endpoint and Behavior identities, and preserve unresolved DI, AOP, reflection, generated, and framework boundaries. Do not publish a whole-repository class inventory.
11. Link each behavior only to relevant repository references. Use stable Dependency, Failure, Java binding, and Config impact IDs in affected Tech frontmatter, with concise links to repository-level anchors. Do not duplicate detailed contracts or repository-wide tables inside every behavior.
...
6. Keep internal flow in Tech Behavior and add a direct `Implementation sequence` link to the related Behavior anchor. Do not copy controllers, classes, database calls, downstream calls, or exception propagation into the Contract. Add a Contract-local Protocol Sequence only when the caller must follow a proven multi-call protocol. Keep detailed endpoint exposure in Endpoint Matrix, outbound HTTP mappings in Field Validation and Mapping, downstream boundaries in External Dependency Contracts, and internal/repository-wide failures in Failure Taxonomy.
