...
Team API Gateway access log groups may use custom names. If multiple API Gateway log groups exist, search likely API Gateway access log groups for the required request ID and use the matching event to identify API ID, stage, resource path, method, and status.
